        ...
        with index.updating(lockspace, file):
            ...
            for recnum in range(MAX_RECORDS):
                ...
                try:
                    ...
                    if res:
                        ...
                        if index.find_record(res.resource) == -1:
                            ...
                            _recreate_record(recnum, offset, index, res)
                            continue
                        else:
                            ...
                            log.debug("Leftover resource found for corrupted "
                                      ...
                                      "resource=%s",
                                      recnum, offset, res.resource)
                            _clear_record(recnum, offset, index)
                            _clear_resource(
                ...
                if record.is_empty():
                    ...
                    if not res:
                        # Record ok, but no resource found - recreate sanlock
                        # resource from record.
                        log.debug(
                            "Found missing sanlock resource recnum=%s "
                            "resource=%s offset=%s",
                            recnum, record.resource, offset)
                        write_resource(
                            lockspace, record, file.name, offset, alignment,
                            block_size)
                        continue
                    if record.resource != res.resource:
                        # Resource and index record exist but IDs mismatch.
                        log.debug(
                            "Repairing lease mismatch. Sanlock resource=%s to "
                            "resource=%s recnum=%s offset=%s",
...
def _recreate_record(recnum, offset, index, res):
    log.debug(
        "Recreating index record from sanlock recnum=%s resource=%s offset=%s",
        recnum, res.resource, offset)
    record = Record(res.resource, offset)
    index.write_record(recnum, record)


def _clear_record(recnum, offset, index):
    log.debug("Clearing index record recnum=%s offset=%s", recnum, offset)
    record = EMPTY_RECORD
    index.write_record(recnum, record)
...
def _clear_resource(
    ...
    log.debug("Clearing sanlock resource file=%s offset=%s", path, offset)
    sanlock.write_resource(
        b"",
        b"",
        [(path, offset)],
...
def write_resource(
        lockspace, record, path, offset, alignment=sc.ALIGNMENT_1M,
        block_size=sc.BLOCK_SIZE_512):
    """
    Helper for writing sanlock resources.
    """
    lockspace = lockspace.encode("utf-8")
    resource = record.resource.encode("utf-8")
    log.debug("Writing sanlock resource=%s lockspace=%s path=%s offset=%s",
