...
def verify_lease_added(lease, lease_id, resource, volume):
...
class TestIndex:
    ...
    def test_updating_record_overwrites(self, tmp_vol, fake_sanlock):
        ...
        tmp_vol.write_records((target_recnum, record))
        vol = xlease.LeasesVolume(
            tmp_vol.backend,
            alignment=tmp_vol.alignment,
            block_size=tmp_vol.block_size)
        # Lease.add() should find that there already is a record in updating
        # state and use it instead of creating a new one. The updating flag
        # will be removed on purpose.
        with utils.closing(vol):
            lease = vol.add(lease_id)
            res = fake_sanlock.read_resource(
                lease.path,
